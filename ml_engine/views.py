...
# For label binarization (Bug C2 fix: was used but never imported)
...
def train_model_view(request):
    # =========================================================================
    # DATA SOURCE: Use Scaled Dataset if available, otherwise use Final Dataset
    # =========================================================================
    # This ensures scaled features are used for model training when Feature Scaling
    # has been completed in the preprocessing pipeline
    
    if "scaled_dataset" in request.session:
        print("\n" + "=" * 70)
        print("TRAINING WITH SCALED DATA (from Feature Scaling step)")
        print("=" * 70)
        dataset_key = "scaled_dataset"
    elif "final_dataset" in request.session:
        dataset_key = "final_dataset"
    else:
        messages.error(request, "Complete feature selection first.")
        return redirect("feature_selection")

    # Load dataset from session - use helper function to handle file references
    try:
        from preprocessing.views import get_dataset
        df = get_dataset(request, dataset_key, dataset_key)
        if df is None:
            messages.error(request, "Dataset not found. Please complete feature selection first.")
            return redirect("feature_selection")
    ...
    if request.method == "GET":
        data_source = "Scaled Dataset" if dataset_key == "scaled_dataset" else "Final Dataset"
...
def train_classification_models(X_train, X_test, y_train, y_test, target, feature_columns, n_classes, request):
    ...
    for name, model in models.items():
        ...
        # Bug C1 fix: was using X_test (unencoded) — must use X_test_encoded
        try:
            if hasattr(model, "predict_proba"):
                ...
                if n_classes == 2:
                    ...
                    y_test_bin = label_binarize(
                        y_test, classes=sorted(y_test.unique().tolist())
                    )
    ...
    # Store detailed model results for report
    detailed_results = {}
    ...
    request.session["model_results"] = detailed_results
...
def calculate_cm_metrics(cm_array, class_labels):
    ...
    # Bug H10 fix: use np.sum() instead of Python's built-in sum().
    # sum(cm_array_np[:, i]) on a 2-D numpy array returns a numpy scalar
    # but sum(sum(cm_array_np)) returns a 1-D array — both produce
    # incorrect results.  np.sum() is explicit and correct.
    total = int(np.sum(cm_array_np))

    for i in range(len(class_labels)):
        tp = int(cm_array_np[i, i])
        fp = int(np.sum(cm_array_np[:, i]) - tp)
        fn = int(np.sum(cm_array_np[i, :]) - tp)
        tn = total - tp - fp - fn
        cm_metrics.append({
...
# =====================================================
# PREDICTION HELPER: Load Scaler for Prediction
# =====================================================
def load_scaler_for_prediction(request):
    """
    Load the fitted scaler from file for applying to new prediction data.
    
    This is the PRODUCTION-READY approach for scaling prediction inputs:
    1. Load the complete fitted scaler object from file
    2. Apply scaler.transform() to raw input data
    3. Supports all scaler types (StandardScaler, MinMaxScaler, RobustScaler)
    4. No data leakage - scaler is NOT refit on prediction data
    
    Returns:
        Tuple of (scaler, scaler_method) or (None, None) if not available
    """
    scaler_path = request.session.get("scaler_path")
    
    if scaler_path and os.path.exists(scaler_path):
        try:
            # Load the complete fitted scaler object
            scaler = joblib.load(scaler_path)
            scaler_method = request.session.get("scaler_params", {}).get("method", "standard")
            
            print("\n" + "=" * 70)
            print("SCALER LOADED FROM FILE (Production-Ready)")
            print("=" * 70)
            ...
            return scaler, scaler_method
        except Exception as e:
            print(f"\n[WARNING] Failed to load scaler from file: {e}")
            print("Falling back to manual parameter application...")
    
    return None, None


def apply_scaling_to_input(input_df, feature_columns, scaler):
    """
    Apply fitted scaler to input data for prediction.
    
    This function:
    1. Extracts only numeric features (in correct order)
    2. Applies scaler.transform() to scale the data
    3. Returns the scaled DataFrame ready for prediction
    
    Args:
        input_df: DataFrame with raw input values
        feature_columns: List of feature column names (in training order)
        scaler: Fitted sklearn scaler object
    
    Returns:
        Scaled DataFrame with same shape as input
    """
    # Extract only numeric features in the correct order
    numeric_features = [col for col in feature_columns if col in input_df.columns]
    
    # Create a DataFrame with only the numeric features in correct order
    X_scaled = input_df[numeric_features].copy()
    
    # Apply the fitted scaler's transform method
    # This uses the exact same scaling that was fit during training
    X_scaled[numeric_features] = scaler.transform(X_scaled[numeric_features])
    
    print("\n[SCALING APPLIED TO PREDICTION INPUT]")
    print("-" * 50)
    for col in numeric_features[:5]:  # Show first 5 features
        original = input_df[col].values[0]
        scaled = X_scaled[col].values[0]
        print(f"  {col}: {original:.4f} -> {scaled:.4f}")
...
def predict_view(request):
    ...
    scaler_params = request.session.get("scaler_params", None)
    ...
    if request.method == "POST":
        ...
        # Bug C9 fix: prediction input must not blindly cast all values to float.
        # Non-numeric (categorical) features that went through OrdinalEncoder
        # during training need to be passed as strings so the model can handle
        # them. We attempt float first (covers all numeric features) and fall
        # back to the raw string value for categorical ones.  The model itself
        # (trained with OrdinalEncoder → int64) will receive numeric data
        # because the same encoding was applied at training time via
        # encode_features_strict(); at prediction time these encoded int values
        # are what was saved in feature_columns.
        try:
            for col in feature_columns:
                raw = request.POST.get(col)
                if raw is None:
                    messages.error(request, f"Missing value for feature: {col}")
                    return redirect("predict")
                try:
                    input_data[col] = float(raw)
                except (TypeError, ValueError):
                    # Keep as string; the model saw OrdinalEncoded integers
                    # for this column — pass the raw string and let the model
                    # handle it (predict() internally calls transform).
                    input_data[col] = raw
        except Exception:
            messages.error(request, "Invalid input values. Please check all fields and try again.")
            return redirect("predict")

        input_df = pd.DataFrame([input_data])
        
        # =========================================================================
        # APPLY SCALING TO NEW INPUT DATA (if scaler params exist)
        # =========================================================================
        # This ensures new predictions use the same scaling that was applied
        # during training, maintaining pipeline consistency
        
        if scaler_params and 'scaler_mean' in scaler_params and 'scaler_scale' in scaler_params:
            print("\n" + "=" * 70)
            print("APPLYING SCALING TO NEW INPUT DATA FOR PREDICTION")
            print("=" * 70)
            
            # Apply StandardScaler manually using saved parameters
            scaler_mean = scaler_params['scaler_mean']
            scaler_scale = scaler_params['scaler_scale']
            
            for col in feature_columns:
                if col in scaler_mean and col in scaler_scale:
                    # z-score normalization: (x - mean) / scale
                    input_df[col] = (input_df[col] - scaler_mean[col]) / scaler_scale[col]
                    print(f"  Scaled {col}: {input_data[col]} → {input_df[col].values[0]:.4f}")
    ...
    return render(request, "ml_engine/predict.html", {
        ...
        "scaled_data_used": scaler_params is not None,
    })
